...
use edphp\db\exception\DataNotFoundException;
use edphp\db\exception\ModelNotFoundException;
use edphp\Exception;
use edphp\exception\DbException;
...
class Query
{
    ...
    public function __call($method, $args)
    {
        ...
        if (isset(self::$extend[strtolower($method)])) {
            ...
            return invoke(self::$extend[strtolower($method)], $args);
        } elseif ($fastKey) {
            return $this->parseFastExpression($method, $fastKey, $args);
    ...
    private function fastMatch($method) {
        foreach ($this->fastMethod as $v) {
            if (strpos($method, $v) === 0) {
                return $v;
            }
        }
    }
    ...
    private function parseFastExpression($method, $fastKey, $args) {
        ...
        foreach ($arr as $field) {
            if ($logic) {
                if (!array_search($field, $this->fastLogic)) {
                    throw new Exception('fast method parse failed, cannot find logic:' . $field);
                }
                $logic = false;
            } else {
                switch($logicField) {
    ...
    public function count($field = '*')
    {
        if (isset($this->options['group'])) {
            ...
            $subSql  = $this->options($options)->field('count(' . $field . ') AS edphp_count')->bind($this->bind)->buildSql();
    ...
    public function getOne($data = null)
    {
        ...
        $result = $this->connection->getOne($this);
    ...
    {
        return $this->failException(true)->getOne($data);
    }
    ...
    {
        return $sub ? '( ' . $this->get(false) . ' )' : $this->get(false);
    }
